function MakeTableHtml(x){
	...
	const colname = ["charname","rarity"];
	//配列データからテーブルHTMLコードを作成
	var xy = "";
	//xy += '<style>.charname{background-color:red;}.rarity{background-color:blue;/*display:none;*/}</style>';
	xy += "<table border='1px' id='GirlsTable'>";
	for(var index=0;index<colname.length;index++){
		i = colname[index];
		xy += ("<colgroup span='1' class='" +i+ "'></colgroup>");
		xy += ('<style>.' + i + '{background-color:red;}</style>');
	}
 ...
 //  テーブルソートの共通変数　設定要！
 //===============================================================
var gTabldID = 'GirlsTable';  // テーブルのエリアのIDを設定
...
function tSortInit(){
 ...
 //===============================================================
  ...
  var wTR     = wTABLE.rows;<!-- jQuery無しでもオブジェクトみたいに扱えるってことか -->
  var wAddBtn = '';
